...
int Partition::TriangulateMonotone(Poly *inPoly, list<Poly> *triangles) {
	...
	push(priority[0], &root);
	push(priority[1], &root);

	for(i = 2; i < (numpoints-1); i++) {
		vindex = priority[i];
		/// on different chains
		if(vertextypes[vindex]!=vertextypes[peek(&root)]) {
			long popped_index;
			while(!isEmpty(root)) {
				popped_index = pop(&root);
				if(isEmpty(root)) break; // Skipping the last element
				if(vertextypes[vindex] == -1) {
					triangle.Triangle(points[vindex],points[peek(&root)],points[popped_index]);
				} else {
					triangle.Triangle(points[vindex],points[popped_index],points[peek(&root)]);
				}
				triangles->push_back(triangle);
			}
			push(priority[i-1], &root);
			push(vindex, &root);
		}
		/// on the same chain
		else {
			...
			while(!isEmpty(root)) {
				popped_index = pop(&root);
				if(vertextypes[vindex] == -1) {
					if(IsConvexAngle(points[vindex],points[peek(&root)],points[popped_index])) {
						triangle.Triangle(points[vindex],points[peek(&root)],points[popped_index]);
						triangles->push_back(triangle);
					} else {
						break;
					...
					if(IsConvexAngle(points[vindex],points[popped_index],points[peek(&root)])) {
						triangle.Triangle(points[vindex],points[popped_index],points[peek(&root)]);
						triangles->push_back(triangle);
					} else {
						break;
	...
	while(!isEmpty(root)) {
		long popped_index = pop(&root);
		if(isEmpty(root)) break; // discard last
		if(vertextypes[popped_index] == -1) {
			triangle.Triangle(points[popped_index],points[peek(&root)],points[vindex]);
		} else {
			triangle.Triangle(points[peek(&root)],points[popped_index],points[vindex]);
		}
		triangles->push_back(triangle);
	}
